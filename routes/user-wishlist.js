...
const {
    DB_ERROR_WISHLIST_FETCH,
    DB_ERROR_WISHLIST_UPDATE
} = require("../lib/error/errors.json");

const SQL_FETCH_WISHLIST = 'CALL SP_fetch_wishlist(?)';
const SQL_UPDATE_WISHLIST = 'CALL SP_update_wishlist(?,?,?)';

router.get('/',
    validateToken,
    (req, res) => {
        logger.debug("Fetch user wishlist detail");
        const { userId } = req._data.middleware;
        db.query(SQL_FETCH_WISHLIST, [userId],
            (err, results) => {
                if (err) {
                    // TODO
                    return sendError(res, DB_ERROR_WISHLIST_FETCH, err);
                }
                else {
                    const wishlist = results[0].map(item => {
                        return {
                            id: item.id,
							name: item.name,
							image: item.image,
...
router.post('/:variantId/:quantity',
    validateToken,
    (req, res) => {
        logger.debug("Update user wishlist");
        const { userId } = req._data.middleware;
        const { variantId, quantity } = req.params;
        db.query(SQL_UPDATE_WISHLIST, [userId, variantId, quantity],
            (err, results) => {
                if (err) {
                    // TODO
                    return sendError(res, DB_ERROR_WISHLIST_UPDATE, err);
                }
                else {
